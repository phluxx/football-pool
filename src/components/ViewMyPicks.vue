...
export default {
  ...
  try {
    ...
    async fetchUserTiebreaker() {
      try {
        const response = await axios.get(`https://fbpsql.ewnix.net/api/getusertiebreaker/${this.decodedUsername}/${this.nextSaturday}`);
        if(response.data) {
          this.tiebreakerAnswer = response.data.tiebreaker;
          this.tiebreakerID = response.data.qid;
        }
      } catch (error) {
        console.error("Error fetching user tiebreaker:", error);
        alert("Error fetching user tiebreaker!");
      }
    }
  }
};
